    ...
    <div className="w-full" {...getRootProps()}>
      <motion.div
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          {/* upload file ui  */}
          <div className="relative w-3/12 h-38">
            {/* {files.length > 0 &&
              files.map((file, idx) => (
                <motion.div
                  key={"file" + idx}
      ...
      {Array.from({ length: rows }).map((_, row) =>
        Array.from({ length: columns }).map((_, col) => {
          ...
          return (
            <div
              key={`${col}-${row}`}
              className={`w-9 h-9 flex shrink-0 rounded-[2px] ${
                index % 2 === 0
                  ? "bg-gray-50 dark:bg-blue-950 hover:bg-cyan-600"
                  : "bg-gray-50 dark:bg-fuchsia-950 hover:bg-cyan-600 shadow-[0px_0px_1px_3px_rgba(255,255,255,1)_inset] dark:shadow-[0px_0px_1px_3px_rgba(0,0,0,1)_inset]"
              }`} />
          );
        }))}
    </div>
